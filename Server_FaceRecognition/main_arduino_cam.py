...
import serial
import numpy as np  # Adicionando a importação necessária

# Inicialize a conexão serial com o Arduino
...
def atualizar_camera():
    global frame_atual
    # Ler os dados enviados pelo Arduino
    dados_arduino = arduino.readline().decode().strip()
    # Converta os dados recebidos em uma matriz numpy
    frame = cv2.imdecode(np.frombuffer(dados_arduino, dtype=np.uint8), -1)
